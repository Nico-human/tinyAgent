    ...
    def wrap_tool_call(
        ...
        if tool_name == "bash":
            ...
            if not isinstance(command, str):
                return ToolMessage(
                    content="Invalid argument: command must be a string",
            ...
            if bool(
                app_context.block_command.destructive_command_word.search(command)
            ) or any(kw in command for kw in app_context.block_command.destructive):
            ...
            if not isinstance(path, str):
                return ToolMessage(
                    content="Invalid argument: path must be a string",
                    tool_call_id=request.tool_call["id"],
                    status="error",
                )
