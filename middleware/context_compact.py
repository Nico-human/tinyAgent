...
from langchain.agents.middleware import (
    AgentMiddleware, ExtendedModelResponse, ModelRequest, ModelResponse,
)
...
class ContextCompactorMiddleware(AgentMiddleware[ContextCompactState, AppContext, Any]):
    ...
    def __init__(self, model: BaseChatModel, *, context_char_limit: int = CONTEXT_CHAR_LIMIT):
        super().__init__()
    ...
    def estimate_chars(messages: list[BaseMessage]) -> int:
        ...
        return len(json.dumps([
            {"role": m.type, "content": m.content,
             ...
             "tool_call_id": m.tool_call_id if isinstance(m, ToolMessage) else None}
            for m in messages
        ], ensure_ascii=False, default=str))

    @staticmethod
    def output_text(message: ToolMessage) -> str:
        return (message.content if isinstance(message.content, str)
                else json.dumps(message.content, ensure_ascii=False))

    @staticmethod
    def write_transcript(messages: list[BaseMessage], workspace: WorkspaceContext) -> Path:
    ...
    @staticmethod
    def persisted_output_path(message: ToolMessage, workspace: WorkspaceContext) -> Path | None:
        ...
        return path if path.resolve().is_relative_to(directory) and path.is_file() else None

    @staticmethod
    def unseen_tool_result_positions(messages: list[BaseMessage]) -> list[int]:
        last_ai = next((i for i in range(len(messages) - 1, -1, -1)
                        if isinstance(messages[i], AIMessage)), -1)
        return [i for i in range(last_ai + 1, len(messages)) if isinstance(messages[i], ToolMessage)]

    def tool_result_budget(self, messages: list[BaseMessage], workspace: WorkspaceContext) -> list:
        messages = list(messages)
        batch = self.unseen_tool_result_positions(messages)
        total = sum(len(self.output_text(messages[i])) for i in batch)
        for i in sorted(batch, key=lambda i: len(self.output_text(messages[i])), reverse=True):
    ...
    @staticmethod
    def safe_cut(messages: list[BaseMessage], cut: int, *, forward: bool = False) -> int:
        ...
        while True:
            ...
            if not crossing:
                return cut
            cut = max(end + 1 for _, end in crossing) if forward else min(start for start, _ in crossing)

    def persist_result(self, message: ToolMessage, workspace: WorkspaceContext,
                       preview_chars: int = 0) -> ToolMessage:
        ...
        if path is None:
            ...
            directory.mkdir(parents=True, exist_ok=True)
            safe_id = re.sub(r"[^A-Za-z0-9._-]", "_", message.tool_call_id)[:80] or "unknown"
        ...
        if preview_chars:
            ...
            replacement = (f"<persisted-output>\nFull output: {path}\n"
                           f"Preview:\n{preview}\n</persisted-output>")
        else:
        ...
        return message.model_copy(update={
            "content": replacement,
            "additional_kwargs": {**message.additional_kwargs, "compact_output_path": str(path)},
        })

    def snip_compact(self, messages: list[BaseMessage], workspace: WorkspaceContext,
                     active: HumanMessage | None) -> list:
        ...
        marker = HumanMessage(f"[{len(middle)} messages archived at {path}; reference only]")
        return [*messages[:head_end], marker, *messages[tail_start:]]

    def micro_compact(self, messages: list[BaseMessage], workspace: WorkspaceContext, target: int) -> list:
        messages = list(messages)
        unseen = set(self.unseen_tool_result_positions(messages))
        consumed = [i for i, m in enumerate(messages) if isinstance(m, ToolMessage) and i not in unseen]
    ...
    def fit_tool_results(self, messages: list[BaseMessage], workspace: WorkspaceContext, target: int) -> list:
        messages = list(messages)
        results = [i for i, m in enumerate(messages) if isinstance(m, ToolMessage)]
        for i in sorted(results, key=lambda i: len(self.output_text(messages[i])), reverse=True):
            if self.estimate_chars(messages) <= target:
                break
    ...
    def compact_history(self, messages: list[BaseMessage], workspace: WorkspaceContext,
                        active: HumanMessage | None, *, reactive: bool = False) -> list:
        transcript = self.write_transcript(messages, workspace)
        cut = self.safe_cut(messages, max(0, len(messages) - self.KEEP_RECENT_MESSAGES)) if reactive else 0
        ...
        if not summary.text.strip():
            raise ValueError("Context compaction returned an empty summary; history was not replaced")
    ...
    def is_context_error(error: Exception) -> bool:
        ...
        return any(marker in text for marker in (
            ...
            "too many tokens", "context window exceeded", "prompt is too long",
        ))
    ...
    def retry_response(self, messages: list[BaseMessage], response: ModelResponse) -> ExtendedModelResponse:
        ...
        return ExtendedModelResponse(
            ...
            command=Command(update=self.replace_messages([*messages, *response.result])),
        )

    def before_agent(self, state: ContextCompactState, runtime: Runtime[AppContext]) -> dict:
        # Capture before TodoMiddleware can inject a HumanMessage reminder.
        active = next((m for m in reversed(state["messages"]) if isinstance(m, HumanMessage)), None)
        return {"compact_request": active, "compact_requested": False}

    async def abefore_agent(self, state: ContextCompactState, runtime: Runtime[AppContext]) -> dict:
        return self.before_agent(state, runtime)

    def before_model(self, state: ContextCompactState, runtime: Runtime[AppContext]) -> dict | None:
    ...
    async def abefore_model(self, state: ContextCompactState, runtime: Runtime[AppContext]) -> dict | None:
        return await asyncio.to_thread(self.before_model, state, runtime)

    def wrap_model_call(self, request: ModelRequest, handler: Callable) -> ModelResponse | ExtendedModelResponse:
        ...
        messages = self.compact_history(
            request.messages, request.runtime.context.workspace,
            request.state.get("compact_request"), reactive=True,
        )
        response = handler(request.override(messages=messages))  # One retry; further errors propagate.
        return self.retry_response(messages, response)

    async def awrap_model_call(self, request: ModelRequest, handler: Callable) -> ModelResponse | ExtendedModelResponse:
        request = self.with_system_prompt(request)
        try:
        ...
        messages = await asyncio.to_thread(
            self.compact_history, request.messages, request.runtime.context.workspace,
            request.state.get("compact_request"), reactive=True,
        )
