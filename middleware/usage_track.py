...
class UsageTrackMiddleware(AgentMiddleware[UsageTrackState, AppContext, Any]):
    ...
    def before_agent(
        self, state: UsageTrackState, runtime: Runtime[AppContext]
    ) -> dict[str, Any] | None:
        return {
    ...
    def wrap_model_call(
        ...
        cache_hits = state.get("last_turn_cache_tokens", 0) + usage.get(
            "input_token_details", {}
        ).get("cache_read", 0)
    ...
    def after_agent(
        ...
        print(
            f"\033[90m[HOOK] last_turn_input_tokens: {state['last_turn_input_tokens']}\033[0m"
        )
        print(
            f"\033[90m[HOOK] last_turn_output_tokens: {state['last_turn_output_tokens']}\033[0m"
        )
        print(
            f"\033[90m[HOOK] last_turn_total_tokens: {state['last_turn_total_tokens']}\033[0m"
        )
        print(
            f"\033[90m[HOOK] last_turn_cache_tokens: {state['last_turn_cache_tokens']}\033[0m"
        )
        return {
            "total_tokens": state.get("total_tokens", 0)
            + state["last_turn_total_tokens"],
            "total_input": state.get("total_input", 0)
