...
from state import TodoItem, TodoState
...
class TodoMiddleware(AgentMiddleware[TodoState, AppContext, Any]):
    ...
    def __run_todo_write(
        ...
        return Command(
            update={
                "todos": validated,
                "messages": [
                    ToolMessage(content=output, tool_call_id=runtime.tool_call_id)
                ],
            }
        )

    def before_agent(
        self, state: TodoState, runtime: Runtime[AppContext]
    ) -> dict[str, Any]:
    ...
    def after_model(
        ...
        calls = (
            [c for c in last_ai.tool_calls if c["name"] == "todo_write"]
            if last_ai
            else []
        )
        if len(calls) > 1:
